...
var hose_length = 30
var hose_segment_length = 10

func _ready():
	var previous_segment = null
	for i in range(hose_length):
		var segment_instance = hose_segment.instantiate()
		segment_instance.position = Vector2(0, hose_segment_length * i)
		add_child(segment_instance)
		if previous_segment != null:
			var joint = PinJoint2D.new()
			joint.position = Vector2(0, hose_segment_length * (i - 1) + hose_segment_length / 2)
		...
		if i == 0:
			...
			mainJoint.position = Vector2(0, -(hose_segment_length / 2))
